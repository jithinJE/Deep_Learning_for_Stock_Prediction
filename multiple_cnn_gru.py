...
def build_cnn_layers(layers, lstm_units, ksize, stride_1, filter_num, cnn_layers, single_lstm_layer):
    ...
    if (not single_lstm_layer):
        branch.add(GRU(lstm_units,input_shape = (layers[1], layers[0]), activation='tanh', return_sequences=True))        
        branch.add(Dropout(dropout/2))

    branch.add(GRU(lstm_units,activation='tanh', return_sequences=False))
